...
v <- create_vocabulary(it) %>% prune_vocabulary(doc_proportion_max = 0.7, doc_proportion_min = 0.01, term_count_min = 5)
vectorizer <- vocab_vectorizer(v)

dtm.paragraph <- create_dtm(it.paragraph, vectorizer)
dtm.question <- create_dtm(it.question, vectorizer)
par.tbl <- as.tibble(as.matrix(dtm.paragraph))
names(par.tbl) <- paste0('par_', names(par.tbl))
que.tbl <- as.tibble(as.matrix(dtm.question))
names(que.tbl) <- paste0('que_', names(que.tbl))
train.mat <- cbind(par.tbl, que.tbl)
rm(par.tbl, que.tbl)
...
que.tbl <- as.tibble(as.matrix(dtm.question))
names(que.tbl) <- paste0('que_', names(que.tbl))
test.mat <- cbind(par.tbl, que.tbl)
rm(par.tbl, que.tbl)
...
readr::write_rds(train.mat, 'data/train_mat.rds')
readr::write_rds(test.mat, 'data/test_mat.rds')
```

```{r, include=FALSE}
rm(v, dtm.paragraph, dtm.question, it, it.paragraph, it.question, sw, sw.url, text_modify, vectorizer,
   tfidf, dtm.tfidf.par, dtm.tfidf.que, lsa, dtm.tfidf.par.lsa, dtm.tfidf.que.lsa, subst, train.mat, test.mat)
gc()
```
