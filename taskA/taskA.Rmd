...
Загрузим промежуточный результат
```{r}
train.data <- readr::read_rds('data/train.rds')
test.data <- readr::read_rds('data/test.rds')
```

Поиграем с полными данными
```{r}
all.data <- union_all(train.data, test.data)

inter_words <- function(text1, text2) {
  res <- paste(intersect(str_split(text1, ' ', simplify = T), 
                         str_split(text2, ' ', simplify = T)), collapse = ' ')
  return(res)
}

all.data$inter_words <- all.data %>% 
  select(paragraph, question) %>% 
  apply(1, function(x){
    inter_words(x[1], x[2])
  })

it.inter_worsd <- itoken(all.data$inter_words, progressbar = FALSE)
it <- itoken(c(all.data$paragraph, all.data$question), progressbar = FALSE)
v <- create_vocabulary(it) %>% prune_vocabulary(doc_proportion_max = 0.3, term_count_min = 5)
vectorizer <- vocab_vectorizer(v)
dtm.inter_words <- create_dtm(it.inter_worsd, vectorizer)
...
```{r}
train.data %<>% 
  mutate(cos_sim = stringsim(paragraph, question, method = 'cosine'),
         lv_dist = stringdist(paragraph, question, method = 'lv'))
test.data %<>% 
  mutate(cos_sim = stringsim(paragraph, question, method = 'cosine'),
         lv_dist = stringdist(paragraph, question, method = 'lv'))
```
