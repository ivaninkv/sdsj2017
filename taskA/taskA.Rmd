...
Данные, предобработанные в Python:
```{r, eval=FALSE, include=FALSE}
train.data <- readr::read_csv('data/train.csv')
test.data <- readr::read_csv('data/test.csv')
```
...
train.data$paragraph %<>% text_modify(T)
train.data$question %<>% text_modify(T)
test.data$paragraph %<>% text_modify(T)
test.data$question %<>% text_modify(T)
```

```{r, eval=FALSE, include=FALSE}
# Запишем данные для обработки их в Python:
readr::write_csv(train.data, 'data/train_task1.csv')
readr::write_csv(test.data, 'data/test_task1.csv')
```
